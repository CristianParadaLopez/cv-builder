...
import {
  ...
  Palette, User, Briefcase, Wrench, Rocket, FileDown, Loader2
} from "lucide-react";
...
export default function Builder({ dark, setDark }: Props) {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      {/* HEADER */}
      <header className="sticky top-0 z-50 border-b px-6 py-4 flex items-center justify-between gap-4"
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>Generador de CV</p>
          </div>
        </div>
              ...
              <div key={n} className="flex items-center gap-1">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all
                    ${active ? "text-white shadow-md" : done ? "cursor-pointer" : "cursor-default"}`}
                  style={{
                    background: active ? "linear-gradient(135deg, var(--accent-1), var(--accent-2))" :
                      done ? "var(--bg-card2)" : "transparent",
      ...
      {/* SUCCESS TOAST */}
      {success && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 text-white px-6 py-3.5 rounded-2xl shadow-2xl text-sm font-semibold flex items-center gap-2 animate-slide-up"
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-12">
        ...
        {step <= 4 && (
          <div className="text-center mb-12 animate-slide-up">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-5"
              style={{ background: "var(--bg-card2)", border: "1px solid var(--border)", color: "var(--text-muted)" }}>
              {(() => { const Icon = stepLabels[step-1].icon; return <Icon size={14} style={{ color: "var(--accent-1)" }} />; })()}
              Paso {step} de 5
            </div>
            <h2 className="text-4xl lg:text-5xl font-black tracking-tight mb-3">
              {["Elegí tu diseño", "Datos personales", "Experiencia y educación", "Habilidades y herramientas", "Tu CV está listo"][step - 1]}
            </h2>
        ...
        {step === 5 && (
          <div className="text-center mb-12 animate-slide-up">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-5"
              style={{ background: "var(--bg-card2)", border: "1px solid var(--border)", color: "var(--text-muted)" }}>
              <Rocket size={14} style={{ color: "var(--accent-2)" }} />
              Paso 5 de 5
            </div>
            <h2 className="text-4xl lg:text-5xl font-black tracking-tight mb-3">Tu CV está listo</h2>
            <p className="text-lg" style={{ color: "var(--text-muted)" }}>Revisá tu diseño y exportalo en PDF</p>
          </div>
        )}
        ...
        {(step === 2 || step === 3 || step === 4) && (
          <div className="max-w-5xl mx-auto animate-slide-up">
            <CVForm step={step} setStep={setStep} onSubmit={handleGenerateWithSuccess} loading={loading} />
        ...
        {step === 5 && (
          <div className="animate-slide-up">
            {error && (
              <div className="max-w-3xl mx-auto rounded-2xl p-5 mb-8 text-red-400 text-sm"
